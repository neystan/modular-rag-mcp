...
class AzureVisionLLMError(RuntimeError):
    """Azure Vision LLM 调用错误。"""


class AzureVisionLLM(BaseVisionLLM):
    """基于 Azure OpenAI Chat Completions 的 Vision LLM。"""
    ...
    def chat_with_image(
        ...
        if not isinstance(text, str) or not text.strip():
            raise AzureVisionLLMError("azure vision input error: text is required")
        ...
        try:
            ...
            raise AzureVisionLLMError(f"azure vision network error: {exc.reason}") from exc
        except (TimeoutError, socket.timeout) as exc:
            raise AzureVisionLLMError(f"azure vision timeout error: {type(exc).__name__}") from exc
        except json.JSONDecodeError as exc:
            raise AzureVisionLLMError("azure vision response error: invalid JSON") from exc
        ...
        metadata = {
            ...
            "api_version": self.config.get("api_version", self.default_api_version),
    ...
    def preprocess_image(self, image_path: str | bytes) -> str:
        """将图片输入标准化为可直接发送给 Azure 的 data URL。"""
    ...
    def _read_image_bytes(self, image_input: str | bytes) -> tuple[bytes, str, str]:
        ...
        if isinstance(image_input, bytes):
            ...
            except (UnicodeDecodeError, AzureVisionLLMError):
                return image_input, "image/png", "bytes"
            return decoded, "image/png", "base64"

        raise AzureVisionLLMError("azure vision input error: image_path must be file path, base64 string, or bytes")
    ...
    def _resize_image(self, image_bytes: bytes, mime_type: str) -> tuple[bytes, dict[str, tuple[int, int]]]:
        ...
        if max_image_size <= 0:
            raise AzureVisionLLMError("azure vision config error: max_image_size must be positive")
        ...
        try:
            ...
            raise AzureVisionLLMError("azure vision input error: unsupported image format") from exc
    ...
    def _chat_endpoint(self) -> str:
        endpoint = str(
            self.config.get("azure_endpoint", self.config.get("endpoint", self.config.get("base_url", "")))
        ).rstrip("/")
        if not endpoint:
            raise AzureVisionLLMError("azure vision config error: azure_endpoint is required")
        return (
            f"{endpoint}/openai/deployments/{self._deployment_name()}/chat/completions"
            f"?api-version={self.config.get('api_version', self.default_api_version)}"
        )

    def _deployment_name(self) -> str:
        deployment_name = self.config.get("deployment_name") or self.config.get("deployment") or self.config.get("model")
        if not deployment_name:
            raise AzureVisionLLMError("azure vision config error: deployment_name/model is required")
        return str(deployment_name)

    def _headers(self) -> dict[str, str]:
        api_key = self.config.get("api_key")
        if not api_key:
            raise AzureVisionLLMError("azure vision config error: api_key is required")
        return {"Content-Type": "application/json", "api-key": str(api_key)}
    ...
    def _parse_response_content(self, data: dict[str, Any]) -> str:
        try:
            ...
            raise AzureVisionLLMError(
                "azure vision response error: missing choices[0].message.content"
            ) from exc
        ...
        if isinstance(content, list):
            ...
            if merged:
                return merged
        raise AzureVisionLLMError("azure vision response error: content must be string or text blocks")

    def _build_http_error(self, exc: HTTPError) -> AzureVisionLLMError:
        error_code = ""
        error_message = ""
        try:
        ...
        details = [f"azure vision HTTP error: {exc.code}"]
        if error_code:
            details.append(f"code={error_code}")
        if error_message:
            details.append(error_message)
        return AzureVisionLLMError("; ".join(details))
    ...
    def _decode_data_url(value: str) -> bytes:
        ...
        if not payload:
            raise AzureVisionLLMError("azure vision input error: invalid data URL")
        return AzureVisionLLM._decode_base64_string(payload)
    ...
    def _decode_base64_string(value: str) -> bytes:
        try:
            ...
            raise AzureVisionLLMError("azure vision input error: invalid base64 image") from exc
