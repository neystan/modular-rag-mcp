...
- **配置文件结构示例** (`config/settings.yaml`)：
	...
	rerank:
	  provider: qwen  # none | cross_encoder | llm | qwen
	  model: qwen3-rerank
...
- **评估运行**：选择评估后端（配置默认 / `ragas` / `custom`）与 golden test set，点击运行。
- **指标展示**：以表格展示 `context_precision`、`context_recall`、`faithfulness`、`answer_relevancy` 等指标，并保留 custom evaluator 汇总能力。
- **历史趋势**：对比不同时间的评估结果，观察策略调整的效果。
- **运行反馈**：运行期间展示进度条与当前 case 状态，支持页面内历史评估结果对比。
...
|---------|---------|------|---------|------|
| A1 | 初始化目录树与最小可运行入口 | [x] | 2026-05-04 | 目录骨架、配置文件、Prompt、main.py、uv 虚拟环境已创建 |
| A2 | 引入 pytest 并建立测试目录约定 | [x] | 2026-05-04 | pytest 配置、冒烟测试、fixture 目录已建立 |
| A3 | 配置加载与校验（Settings） | [x] | 2026-05-04 | Settings dataclass、YAML 加载、必填字段校验、4 个单元测试 |

#### 阶段 B：Libs 可插拔层

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| B1 | LLM 抽象接口与工厂 | [x] | 2026-05-04 | BaseLLM + LLMFactory + 6 个单元测试 |
| B2 | Embedding 抽象接口与工厂 | [x] | 2026-05-04 | BaseEmbedding + EmbeddingFactory + 6 个单元测试 |
| B3 | Splitter 抽象接口与工厂 | [x] | 2026-05-04 | BaseSplitter + SplitterFactory + 6 个单元测试 |
...
| B7.4 | Ollama Embedding 实现 | [x] | 2026-05-04 | Ollama Embedding + 工厂注册 + mock HTTP 测试 |
...
| C10 | BatchProcessor | [x] | 2026-05-05 | 双路编码批处理编排 + 稳定顺序 |
| C11 | BM25Indexer（倒排索引+IDF计算） | [x] | 2026-05-05 | 倒排索引持久化 + roundtrip 查询 + 增量更新 |
| C12 | VectorUpserter（幂等upsert） | [x] | 2026-05-05 | 稳定向量 ID 生成 + 幂等 upsert |
| C13 | ImageStorage（图片存储+SQLite索引） | [x] | 2026-05-06 | 文件存储 + SQLite 索引映射 |
| C14 | Pipeline 编排（MVP 串起来） | [x] | 2026-05-06 | 串行摄取编排 + 清晰失败异常 |
| C15 | 脚本入口 ingest.py | [x] | 2026-05-06 | CLI 参数解析 + 目录/单文件摄取 + 跳过逻辑 |

#### 阶段 D：Retrieval MVP

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| D1 | QueryProcessor（关键词提取 + filters） | [x] | 2026-05-06 | 规则提词 + 空 filters 结构 |
...
| D7 | 脚本入口 query.py（查询可用） | [x] | 2026-05-06 | CLI 查询入口 + verbose 输出 + 单元测试 |
...
| E6 | 多模态返回组装（Text + Image） | [x] | 2026-05-07 | 图片 base64 组装 + MCP image content 集成 |

#### 阶段 F：Trace 基础设施与打点

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| F1 | TraceContext 增强（finish + 耗时统计 + trace_type） | [x] | 2026-05-07 | TraceContext finish/to_dict + TraceCollector + 单元测试 |
...
| G5 | Ingestion 追踪页面 | [x] | 2026-05-07 | TraceService + 摄取历史列表 + 阶段耗时展示 |
| G6 | Query 追踪页面 | [x] | 2026-05-07 | Query TraceService + 查询历史过滤 + Dense/Sparse/Rerank 对比 |
...
- **M5（完成阶段 G）**：六页面可视化管理平台就绪，数据可浏览、可管理、链路可追踪。
- **M6（完成阶段 H+I）**：评估体系、MCP E2E、Dashboard 冒烟、README 收口与契约测试全部完成，形成"面试/教学/演示"可复现项目。
